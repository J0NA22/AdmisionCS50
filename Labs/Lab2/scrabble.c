...
// Points assigned to each letter of the alphabet - Puntos asignados para cada letra del alfabeto
...
int compute_score(string word)
{
    ...
    for (int i = 0; i < strlen(word); i++)
    {
        ...
        if (isupper(word[i]))
        {
            //verifica si es capital y busca en el arreglo ascii
            for (int f = 0; f < sizeof(capital_letters); f++)
            {
                if (word[i] == capital_letters[f])
                {
                    //toma su indice y le asigna puntos segun el indice
        ...
        // si la palabra contiene algo que no es alfabeto lo ignora y salta a la siguiente
        else
        {
            i += 1;
        }
